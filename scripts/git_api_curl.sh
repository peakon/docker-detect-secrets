...
curl -X POST -H 'Content-type: application/json' --data {"text":"Potential secret in Github!", "attachments":"[{'color': 'warning', 'title': 'A new potential secret was commited in Github', 'text': '$COMMIT_URL'}]"} $SF_SECURITY_SLACK_WEBHOOK_URL
